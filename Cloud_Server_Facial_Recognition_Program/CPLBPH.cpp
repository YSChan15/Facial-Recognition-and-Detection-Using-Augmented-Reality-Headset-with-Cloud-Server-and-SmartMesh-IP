...
int main ()
{
    ...
    */
    ...
    // START LOCAL BINARY PATTERNS HISTOGRAMS ALGORITHM
    ...
    try{
        model->read("savedAI_LBPH.txt");   // Use to read previously trained AI model
        cout << "Reloaded previously trained AI model." << endl;
    }
    catch (const exception& e){
        model->train(images, labels);
        model->write("savedAI_LBPH.txt");  // Use to save trained AI model
    ...
    while (1) 
    {
        ...
        else if (strcmp(Buf, "SensorData") == 0)
        {
            ...
            int predictedLabel;
            double loss;
            double lossThreshold = 100;
            model->predict(receivedImage, predictedLabel, loss);
            
            cout << "Loss: " << loss << endl;
            
            if (loss > lossThreshold)  // loss represents deviation from the estimated correct label
            {
                predictedLabel = 4;
            }
