...
// Included for OpenCV Eigenface program
...
int main ()
{
    ...
    // START SUPPORT VECTOR MACHINE PROGRAM
    
    string database[5] = {"Brycen Hillukka\n", "Yu Sheng Chan\n", "Brandon Wieberdink\n", "Hayden Scott\n", "Unknown face\n"};
    ...
    // Train the one-to-many SVMs
    int numSVMs = 4;
    Ptr<SVM> svms[numSVMs];
    Ptr<TrainData> tds[numSVMs];
    vector<int> labels_many[numSVMs];
    ...
    for (int i = 0; i < numSVMs; i++)
    {
        ...
        {
            if (labels[j] == i)
                labels_many[i].push_back(i);   // Known class
            else
                labels_many[i].push_back(5);   // Unknown class
        }
    }
    ...
    {
        svms[i] = SVM::create();
        svms[i]->setType(SVM::C_SVC);
        svms[i]->setKernel(SVM::LINEAR);
        tds[i] = TrainData::create(trainingData, ROW_SAMPLE, labels_many[i]);
        svms[i]->train(tds[i]);
    }
    
    cout << "All AI models trained." << endl;
    ...
    while (1) 
    {
        ...
        if (strcmp(Buf, "Image") == 0)
        {
            ...
            receivedImage = receivedImage.reshape(1, 1);
            receivedImage.convertTo(receivedImage, CV_32F);
        ...
        else if (strcmp(Buf, "SensorData") == 0)
        {
            ...
            int predictedLabel = svm->predict(receivedImage);
            int predictedlabels[numSVMs];
            
            for (int z = 0; z < numSVMs; z++)
            {
                for (int j = 0; j < numSVMs; j++)
                {
                    predictedlabels[j] = svms[j]->predict(receivedImage);
                }
            }
            
            if (predictedLabel == 0 && predictedlabels[0] == 0 && predictedlabels[1] == 5 && predictedlabels[2] == 5 && predictedlabels[3] == 5)
            {
                cout << "Class 0 found" << endl;
                cout << database[predictedlabels[0]] << endl;
            }
            else if (predictedLabel == 1 && predictedlabels[0] == 5 && predictedlabels[1] == 1 && predictedlabels[2] == 5 && predictedlabels[3] == 5)
            {
                cout << "Class 1 found" << endl;
                cout << database[predictedlabels[1]] << endl;
            }
            else if (predictedLabel == 2 && predictedlabels[0] == 5 && predictedlabels[1] == 5 && predictedlabels[2] == 2 && predictedlabels[3] == 5)
            {
                cout << "Class 2 found" << endl;
                cout << database[predictedlabels[2]] << endl;
            }
            else if (predictedLabel == 3 && predictedlabels[0] == 5 && predictedlabels[1] == 5 && predictedlabels[2] == 5 && predictedlabels[3] == 3)
            {
                cout << "Class 3 found" << endl;
                cout << database[predictedlabels[3]] << endl;
            }
            else
            {
                cout << "No class recognized" << endl;
                cout << "Unknown face" << endl;
                predictedLabel = 4;
            }
    
            if (predictedLabel >= 0 && predictedLabel < 5)
            {
                cout <<  "Recognized: " << database[predictedLabel];
                
                usleep(50000); // Delay for AR headset to receive name after OK signal
                ...
                if (numImagesReceived == numImages)
                {
                    ...
                    err = send(SmartMeshSocket, allReceivedNames.c_str(), allReceivedNames.size(), 0);
