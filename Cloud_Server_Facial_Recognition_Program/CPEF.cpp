...
int main ()
{
    ...
    Ptr<EigenFaceRecognizer> model = EigenFaceRecognizer::create(100);
    
    try{
        model->read("savedAI_EF.txt");   // Use to read previously trained AI model
        cout << "Reloaded previously trained AI model." << endl;
    }
    catch (const exception& e){
        model->train(images, labels);
        model->write("savedAI_EF.txt");  // Use to save trained AI model
    ...
    while (1) 
    {
        ...
        else if (strcmp(Buf, "SensorData") == 0)
        {
            ...
            int predictedLabel = model->predict(receivedImage);
            ...
            if (predictedLabel >= 0 && predictedLabel < 4)
            {
                ...
                send(ARSocket, database[predictedLabel].c_str(), database[predictedLabel].size(), MSG_CONFIRM);
